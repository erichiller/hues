...
static int ssl_write_real( mbedtls_ssl_context *ssl,
                           const unsigned char *buf, size_t len )
{
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> marker A\n"));		
	
    int ret;
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> MBEDTLS_SSL_MAX_FRAGMENT_LENGTH\n"));
    ...
    {
#if defined(MBEDTLS_SSL_PROTO_DTLS)
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> MBEDTLS_SSL_PROTO_DTLS\n"));

        if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
        {
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> too large, datagram\n"));
...
#endif
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> marker B\n"));
    ...
    if( ssl->out_left != 0 )
    {
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> marker C\n"));
		
        if( ( ret = mbedtls_ssl_flush_output( ssl ) ) != 0 )
        {
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> marker D; ret=%X\n", ret));
    ...
    {
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> marker E\n"));
        ...
        if( ( ret = mbedtls_ssl_write_record( ssl ) ) != 0 )
        {
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_real ===>> marker F; ret=%X\n", ret));
...
{
    int ret;
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_split ===>> renegotiate\n"));
    ...
    if( ssl->conf->cbc_record_splitting ==
        ...
        mbedtls_cipher_get_cipher_mode( &ssl->transform_out->cipher_ctx_enc )
                                != MBEDTLS_MODE_CBC )
    {
		MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_split ===>> return branch A\n"));
    ...
    if( ssl->split_done == 0 )
    {
        if( ( ret = ssl_write_real( ssl, buf, 1 ) ) <= 0 )
            return( ret );
        ssl->split_done = 1;
		MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_split ===>> marker B\n"));
    }

    if( ( ret = ssl_write_real( ssl, buf + 1, len - 1 ) ) <= 0 )
		MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_split ===>> marker C\n"));
        return( ret );
    ssl->split_done = 0;
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "ssl_write_split ===>> marker D\n"));
    return( ret + 1 );
}
...
#if defined(MBEDTLS_SSL_RENEGOTIATION)
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "mbedtls_ssl_write ===>> renegotiate\n"));
...
#endif
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "mbedtls_ssl_write ===>> checking handshake\n"));
    if( ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER )
    {
        if( ( ret = mbedtls_ssl_handshake( ssl ) ) != 0 )
        {
			MBEDTLS_SSL_DEBUG_MSG( 2, ( "mbedtls_ssl_write ===>> doing handshake\n"));
...
#if defined(MBEDTLS_SSL_CBC_RECORD_SPLITTING)
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "mbedtls_ssl_write ===>> record splitting\n"));

    ret = ssl_write_split( ssl, buf, len );
#else
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "mbedtls_ssl_write ===>> no record splitting\n"));
