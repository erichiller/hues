...
int main(int argc, char *argv[])
{
	...
	const unsigned char psk[] = { 0x32, 0x1c, 0x0c, 0x2e, 0xbf, 0xa7, 0x36, 0x1e, 0x55, 0x49, 0x10, 0x95, 0xb2, 0xf5, 0xf9, 0xdb };
...
send_request:
	...
	while (1) {
		...
		do ret = mbedtls_ssl_write(&ssl, (unsigned char *)message1, len);
		while (ret == MBEDTLS_ERR_SSL_WANT_READ ||
			ret == MBEDTLS_ERR_SSL_WANT_WRITE);
		...
		do ret = mbedtls_ssl_write(&ssl, (unsigned char *)message2, len);
		while (ret == MBEDTLS_ERR_SSL_WANT_READ ||
			ret == MBEDTLS_ERR_SSL_WANT_WRITE);
