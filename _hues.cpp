...
int loop(mbedtls_ssl_context *ssl) {
    logm("creating timer\n");
    unsigned long t_last = mbedtls_timing_get_timer( timer, 1);
    unsigned long t_now;
    logm("timer created\n");
    int ret, len, rx;
    ...
    while (1) {
        ...
        usleep(SLEEP_PERIOD);
        loop_now+=rx;
        t_now = mbedtls_timing_get_timer(timer, 0);
        if ( t_now - t_last > 1000 ) {
            printf("%ims elapsed, %i colors updated", (t_now - t_last), (loop_now - loop_last) );  
            loop_last = loop_now;
        }
    }
    return 0;
...
int main(int argc, char *argv[]) {
   ...
   * 56    3C    8C */
  const unsigned char psk[] = {0xba, 0x2a, 0x27, 0x18, 0x85, 0x63, 0x10, 0xb1,
                               0xbf, 0xf8, 0x46, 0xa2, 0x0d, 0x56, 0x3c, 0x8c};
  ...
  if ((ret = mbedtls_ssl_conf_psk(&conf, psk, sizeof(psk),
