...
timeval      curr_time;
unsigned int time_curr_millis;
unsigned int time_last_millis   = 0;
unsigned int time_period_millis = 1000 / HUE_TX_RATE_HZ;
...
int hue_mbed_tx( uint16_t red, uint16_t green, uint16_t blue ) {
	...
	time_curr_millis = ( curr_time.tv_sec * 1000 ) + ( curr_time.tv_usec / 1000 );
	// ESP_LOGV( LOGT, 
	// "\n\tcurr_time.tv_sec=%lu   (x1000 =%lu)\n"
	// "\tcurr_time.tv_usec=%lu   (/1000 =%lu)\n"
	// "\ttime_curr_millis=%u\n"
	// "\ttime_last_millis=%u\n"
	// "\ttime_period_millis=%u\n"
	// , curr_time.tv_sec,curr_time.tv_sec*1000, curr_time.tv_usec, curr_time.tv_usec/1000, time_curr_millis, time_last_millis, time_period_millis );
	

	if( time_last_millis == 0 || ( time_last_millis <= ( time_curr_millis - time_period_millis ) ) ) {
		...
		// len = sizeof(message);
		len = 25;
		ESP_LOGV( LOGT, "[time=%i][time_period_millis=%u][last=%u] sending message (len=%d) hex:", time_curr_millis, time_period_millis, time_last_millis, len );
		ESP_LOG_BUFFER_HEX_LEVEL( LOGT, message, len, ESP_LOG_VERBOSE );
		...
		if( ret < 0 ) {
			...
			ESP_LOGE( LOGT, " !! WRITE FAILED. mbedtls_ssl_write returned %d (0x%X): %s\n\n", ret, (short)-ret, errbuf );
			return ret;
		}

		time_last_millis = time_curr_millis;
		ESP_LOGV( LOGT, "[time=%i] sent message time_last_millis is now: %i", time_curr_millis, time_last_millis );
...
void exit_close_hue_stream( void ) {
	if( hue_end_stream( ) ) {
		ESP_LOGI( LOGT, "closed hue stream successfully" );
	} else {
...
void hue_mbed_error( int returned_error ) {
	char errbuf[128];
	mbedtls_strerror( returned_error, errbuf, 127 );
	ESP_LOGE( LOGT, "hue_mbed_error reports, there was an error: %d (0x%X): %s\n\n", returned_error, (short)-returned_error, errbuf );
	dtls_conn_exit( );
}
...
int hue_mbed_open_dtls( ) {
	...
	// 16 length                   BA    2A    27    18    85    63    10    B1    BF    F8    46    A2    0D    56    3C    8C
    ...
    */
	...
	if( !hue_begin_stream( ) ) {
		ESP_LOGE( LOGT, "hue entertainment group setup was not successful, dtls stream setup can not continue in %s:%i", __FILE__, __LINE__ )
		return false;
	}
    ...
    */
	...
	mbedtls_esp_enable_debug_log( &conf, LOG_MBED_LEVEL );
#endif
    ...
    * MBEDTLS_SSL_VERIFY_OPTIONAL, we would bail out here if ret != 0 */
	if( ( flags = mbedtls_ssl_get_verify_result( &ssl ) ) != 0 ) {
		...
		ESP_LOGE( LOGT, "mbedtls_ssl_get_verify_result failed\n" );
		...
		// successfully created SSL context
		mbedtls_printf( " ok\n" );
	}
	return success;
}
	...
	if( ret <= 0 ) {
		switch( ret ) {
			case MBEDTLS_ERR_SSL_TIMEOUT:
				...
				if( retry_left-- > 0 ) {
					ESP_LOGE( LOGT, "SSL post-read ; retry NO LONGER IMPLEMENTED" );
				}
				hue_mbed_error( ret );
